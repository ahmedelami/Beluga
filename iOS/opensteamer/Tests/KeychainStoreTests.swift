...
final class KeychainStoreTests: XCTestCase {
    func testProductionStorageIdentityRemainsCompatibleWithEarlierBuilds() {
        // The visible rename must not fork the production Keychain namespace.
    ...
    func testKeychainRoundTripSurvivesStoreRecreation() throws {
        let item = KeychainStore.Item(
            service: "org.example.opensteamer.persistence-tests",
    ...
    func testStoreReadsLegacyGenericPasswordItem() throws {
        let item = KeychainStore.Item(
            service: "org.example.opensteamer.legacy-tests",
            account: UUID().uuidString
        )
    ...
    func testNewItemKeepsLegacyAccessibilityClass() throws {
        let item = KeychainStore.Item(
            service: "org.example.opensteamer.accessibility-tests",
            account: UUID().uuidString
        )
    ...
    func testEmptyValueCannotDeletePersistedItem() throws {
        let item = KeychainStore.Item(
            service: "org.example.opensteamer.deletion-tests",
            account: UUID().uuidString
        )
        let store = KeychainStore(item: item)
        defer { try? store.deleteRemoteToken() }
    ...
    func testExplicitDeleteRemovesPersistedItem() throws {
        let item = KeychainStore.Item(
            service: "org.example.opensteamer.explicit-deletion-tests",
            account: UUID().uuidString
        )
        let store = KeychainStore(item: item)
        defer { try? store.deleteRemoteToken() }
    ...
    #if OPENSTEAMER_UPDATE_SEED
    func testSeedStableItemsForPhysicalUpdateValidation() throws {
        XCTAssertTrue(OpensteamerAppRootMode.isPhysicalUpdateValidationHost)
    ...
    #if OPENSTEAMER_UPDATE_MISSING_IDENTITY_SEED
    ...
    #if OPENSTEAMER_UPDATE_MISSING_IDENTITY_VERIFY
    ...
    #if OPENSTEAMER_UPDATE_MISSING_PAIR_SEED
    ...
    #if OPENSTEAMER_UPDATE_MISSING_PAIR_VERIFY
...
private struct PhysicalUpdatePairingExpectation: Codable, Equatable {
    static let keychainItem = KeychainStore.Item(
        service: "org.example.opensteamer.update-validation",
        account: "viewer-pairing-expectation"
    )
    static let currentSchemaVersion = 3
    // The validation host installs over the pre-rebrand development app by design.
    ...
    static let identityChallenge = Data(
        "opensteamer.PhysicalUpdate.ViewerIdentityChallenge.v1".utf8
    )
...
private func makePhysicalUpdateKeychainItems(
    ...
    let service = "org.example.opensteamer.raw-update-validation-tests.\(testName)"
