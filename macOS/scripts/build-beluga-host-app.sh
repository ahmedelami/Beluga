#!/bin/zsh
# Builds, normalizes, signs, and verifies a fresh Release `Beluga Host.app`.
set -euo pipefail
umask 077

readonly ROOT_DIR="$(cd "$(dirname "$0")/../.." && pwd -P)"
readonly BUNDLE_VERIFIER="$ROOT_DIR/macOS/scripts/verify-beluga-host-bundle.sh"
...
readonly APP_DIR="$APP_OUTPUT_DIR/Beluga Host.app"
...
/bin/chmod 644 "$CONTENTS_DIR/Info.plist" "$RESOURCES_DIR/ThirdPartyNotices.md" "$RESOURCES_DIR/AppIcon.icns"
...
/usr/bin/codesign --force --sign "$SIGNING_IDENTITY" --timestamp=none "$APP_DIR" \
    || fail "could not sign Beluga Host.app"
