...
if (( $# < 1 || $# > 3 )); then
    print -u2 -- \
        "usage: $0 [--installed-runtime] [--media-integration-v1] <opensteamer Host.app> [expected-team-id] [designated-requirement-reference-code]"
    exit 64
fi
...
EXPECTED_RESOURCES_ENTRIES="ThirdPartyNotices.md"
if [[ "$MEDIA_CONTRACT" == 1 ]]; then
